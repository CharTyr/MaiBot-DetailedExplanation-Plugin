...
当需要对科普、技术、概念等复杂问题做“长文解释”时，本插件直接生成结构化长文，并智能分段发送。新版已绕过默认回复管道（不使用回复器的分割/表达加工），更贴近“专栏式”输出，同时保留麦麦的人设风格；在 MaiBot 0.12+（mood 模块移除）下会自动降级情绪注入能力。

## 兼容性与分支说明

- `main`：适配 **MaiBot 0.12+**（`src.mood` 已移除；插件会自动使用默认心情文案，不影响其他功能）
- `maibot-0.11`：适配 **MaiBot 0.11 及更早版本**（使用 `src.mood.mood_manager` 注入真实心情）

## 主要特性

- 智能激活与人设风格融合：在 LLM 判断/关键词触发的基础上，自动注入 bot 人设、表达风格与当前心情（0.12+ 下为默认心情文案），使长文保持麦麦的个性与语气。
...
- 人设与情绪：从 `bot_config.toml` 读取昵称、别名、人设说明、表达风格，并结合当前心情注入提示词使输出更风格化；在 MaiBot 0.12+ 下会自动使用默认心情文案（因为 mood 模块已移除）。
